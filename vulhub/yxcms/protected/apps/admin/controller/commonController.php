   ...
   //信息添加或者编辑时ajax获取拓展字段信息
	protected function extend_field()
	{
		...
		if($extendid==0) echo json_encode($info['extendid']);//判断是否为字段
			...
			$fieldlist = model('extend')->select("pid='{$extendid}'");
			if(!empty($exmesid)){//如果是编辑信息，取出字段值
    ...
    //获取keywords
    protected function getkeyword($title=null,$description=null)
    {
        error_reporting(0);
	...
	//图片批量上传,ajax方式使用
	protected function AjaxUpload($filename,$ifthumb=false)
	{
		...
		$upload->thumbMaxWidth = config('thumbMaxwidth'); // 缩略图最大宽度
		$upload->thumbMaxHeight = config('thumbMaxheight'); // 缩略图最大高度
		$upload->upload(); //上传
