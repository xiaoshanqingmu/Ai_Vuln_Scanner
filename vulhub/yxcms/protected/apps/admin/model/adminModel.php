...
class adminModel extends baseModel{
	...
	public function adminANDgroup($limit=''){
		$sql="SELECT A.id,A.groupid,A.realname,A.lastlogin_time,A.lastlogin_ip,A.iflock,B.name FROM {$this->prefix}admin A,{$this->prefix}group B WHERE A.groupid=B.id AND A.id!='1' ORDER BY A.groupid,A.id LIMIT {$limit}";
